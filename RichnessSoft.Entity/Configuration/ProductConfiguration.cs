...
namespace RichnessSoft.Entity.Configuration
{
    internal class ProductConfiguration : IEntityTypeConfiguration<Product>
    {
        public void Configure(EntityTypeBuilder<Product> builder)
        {
            ...
            builder.Property(e => e.name1).HasColumnType("varchar(160)").IsRequired();
            builder.Property(e => e.sname1).HasColumnType("varchar(160)");
            ...
            builder.Property(e => e.stdcost).HasColumnType("decimal(18,2))");
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.Products).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.ProductGroup).WithMany(p => p.Products).HasForeignKey(e => e.productgroupid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Category).WithMany(p => p.Products).HasForeignKey(e => e.categoryid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Brand).WithMany(p => p.Products).HasForeignKey(e => e.brandid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Format).WithMany(p => p.Products).HasForeignKey(e => e.formatid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Design).WithMany(p => p.Products).HasForeignKey(e => e.designid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Grade).WithMany(p => p.Products).HasForeignKey(e => e.gradeid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Models).WithMany(p => p.Products).HasForeignKey(e => e.modelid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Size).WithMany(p => p.Products).HasForeignKey(e => e.sizeid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Color).WithMany(p => p.Products).HasForeignKey(e => e.colorid).OnDelete(DeleteBehavior.Restrict);
            builder.HasOne(p => p.Weight).WithMany(p => p.Products).HasForeignKey(e => e.weightid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}

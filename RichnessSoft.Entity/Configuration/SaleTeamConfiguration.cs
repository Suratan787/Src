...
{
    internal class SaleTeamConfiguration : IEntityTypeConfiguration<SaleTeam>
    {
        public void Configure(EntityTypeBuilder<SaleTeam> builder)
        {
            builder.ToTable("saleteam");
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.SaleTeams).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
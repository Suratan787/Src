...
{
    internal class MemberGroupConfiguration : IEntityTypeConfiguration<MemberGroup>
    {
        public void Configure(EntityTypeBuilder<MemberGroup> builder)
        {
            builder.ToTable("membergroup");
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.MemberGroups).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
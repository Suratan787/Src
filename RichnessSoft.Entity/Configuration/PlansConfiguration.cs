...
{
    internal class PlansConfiguration : IEntityTypeConfiguration<Plans>
    {
        public void Configure(EntityTypeBuilder<Plans> builder)
        {
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.Plans).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
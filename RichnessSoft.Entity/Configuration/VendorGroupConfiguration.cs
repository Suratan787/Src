...
{
    internal class VendorGroupConfiguration : IEntityTypeConfiguration<VendorGroup>
    {
        public void Configure(EntityTypeBuilder<VendorGroup> builder)
        {
            builder.ToTable("vendorgroup");
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.VendorGroups).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
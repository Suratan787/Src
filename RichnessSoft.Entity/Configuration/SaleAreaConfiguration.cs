...
{
    internal class SaleAreaConfiguration : IEntityTypeConfiguration<SaleArea>
    {
        public void Configure(EntityTypeBuilder<SaleArea> builder)
        {
            builder.ToTable("salearea");
            ...
            builder.HasOne(p => p.Company).WithMany(p => p.SaleAreas).HasForeignKey(e => e.companyid).OnDelete(DeleteBehavior.Restrict);
        }
    }
}
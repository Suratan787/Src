    ...
    public partial class WeightsEdit
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (Id > 0)
            {
                ...
                Weights = (Weight)r.Data;
            }
            else
            {
                Mode = gbVar.ModeInsert;
                Weights = new Weight();
                Weights.companyid = store.CurentCompany.id;
                Weights.active = ConstUtil.ACTIVE.YES;
            }
        }
        ...
        async void SaveAsync()
        {
            ...
            try
            {
                ...
                if (Validated && CheckDupCode())
                {
                    ...
                    {
                        results = weightService.Add(Weights);
                    }
                    else if (Mode == gbVar.ModeEdit)
                    {
                        results = weightService.Edit(Weights);
                    }
                    _loaded = false;
                    if (results.Success)
                    {
        ...
        private bool CheckDupCode()
                { 
                    bool bSucc = true;
                    var res = weightService.GetByCode(Weights.companyid, Weights.code);
                    if (res.Data != null && !string.IsNullOrEmpty(((Weight)res.Data)?.code))
                    {
                        ...
                        else if (Mode == gbVar.ModeEdit && OldData.id != Weights.id)
                        {
                            bSucc = false;
                        }
                    }
